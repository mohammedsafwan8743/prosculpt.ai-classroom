...
export default function AdminClassrooms() {
  ...
  const [editStatus, setEditStatus] = useState<ClassroomStatus>("pending");
  ...
  const { data: trainers = [] } = useQuery<Trainer[]>({
    ...
    enabled: !!editingClassroom, // fetch only when opening modal
  });
  ...
  useEffect(() => {
    if (editingClassroom) {
      ...
      setEditTrainerId(editingClassroom.trainer_id || "");
      setEditStatus(editingClassroom.status || "pending");
      setEditErrorMsg("");
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: any) => {
      setEditErrorMsg(err.response?.data?.error || "Failed to delete classroom.");
    },
  });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        ...
        description="Monitor and view smart classroom records across all college campuses."
      />
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 p-4 backdrop-blur-sm overflow-y-auto animate-fade-in">
          <div className="ps-card w-full max-w-2xl p-6 space-y-6 my-8 animate-scale-in bg-card border border-border rounded-xl shadow-2xl text-left">
            ...
            <form onSubmit={handleEditSubmit} className="space-y-6">
              ...
              {/* Admin allocation & status controls */}
              <div className="grid gap-4 sm:grid-cols-2 bg-muted/20 p-4 rounded-xl border border-border">
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-foreground">Allocated Trainer</label>
                  <select
                    value={editTrainerId}
                  ...
                  >
                    <option value="">Unallocated / No Trainer</option>
                    {trainers.map((t) => (
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-foreground">Classroom Status</label>
                  <select
                    value={editStatus}
                    onChange={(e) => setEditStatus(e.target.value as ClassroomStatus)}
                    className="w-full h-10 rounded-lg border border-input bg-background px-3 text-sm outline-none focus:border-primary"
                  >
              ...
              <div className="border-t pt-4">
                {showDeleteConfirm ? (
                  <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/30 p-4 rounded-lg space-y-3">
                    <p className="text-sm font-semibold text-red-650 dark:text-red-400">
                      Delete this classroom permanently?
                    </p>
                    <p className="text-xs text-muted-foreground">
                      This action cannot be undone. It will remove all student enrollments, sessions, and materials associated with this classroom.
                    </p>
                    <div className="flex gap-2">
                      <button
                        type="button"
                        onClick={() => deleteMutation.mutate(editingClassroom.id)}
                        disabled={deleteMutation.isPending}
                        className="bg-red-600 hover:bg-red-750 text-white text-xs px-3 py-1.5 rounded-lg flex items-center gap-1 font-semibold"
                      >
                      ...
                      <button
                        ...
                        className="btn-outline text-xs px-3 py-1.5 rounded-lg font-semibold"
                      >
                        Cancel
                      </button>
                    </div>
                  ...
                  <button
                    ...
                    className="btn-outline border-error/30 text-error hover:bg-error/5 h-9 px-4 text-xs font-semibold flex items-center gap-1"
                  >
                    <Trash className="h-3.5 w-3.5" />
                    Delete Classroom
                  </button>
...
function AdminClassroomCard({
  ...
  return (
    <div className="ps-card space-y-4 p-5 text-left flex flex-col justify-between">
