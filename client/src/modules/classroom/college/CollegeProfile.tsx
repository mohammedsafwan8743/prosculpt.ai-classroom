  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        ...
        description="View your institutional coordinator information."
      />
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="ps-card flex flex-col items-center p-8 text-center">
          ...
          <h2 className="mt-4 font-display text-xl font-bold text-foreground">
            {user?.email?.split("@")[0].replace("staging.", "").replace(".", " ") ?? "College Coordinator"}
          </h2>
        ...
        <div className="ps-card mt-4 divide-y divide-border">
          <ProfileRow icon={User} label="Coordinator Name" value="College Coordinator" />
          <ProfileRow icon={Mail} label="Contact Email" value={user?.email ?? "—"} />
          <ProfileRow icon={Building2} label="Institution" value="ProSculpt Staging University" />
          <ProfileRow icon={Tag} label="Institution Code" value="STG" />
        </div>
      </div>
    </div>
  );
